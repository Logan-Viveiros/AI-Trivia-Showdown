...
export default function ClassicQuiz({ quiz }: { quiz: Quiz }) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(advanceQuestion, ADVANCE_DELAY_MS);

    return () => window.clearTimeout(timeout);
  }, [advanceQuestion, chosenAnswerIndex]);
        ...
        {currentQuestion.answers.map((answer, answerIndex) => {
          ...
          return (
            <button
              ...
              type="button"
              disabled={isAnswered}
              onClick={() => setChosenAnswerIndex(answerIndex)}
              className={`flex min-h-40 items-center justify-center px-6 py-8 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.12)] transition duration-200 ease-out sm:min-h-56 ${answerState.containerClass} ${
                isAnswered
